...
def getContractConstructorParams(dom, contractName):
    try:
        ...
        # print(dom("pre.wordwrap").eq(2).text.txt())
        # print(constructor.text.txt())
...
def getTable(tableNode):
    ...
    for i in range(len(trs)):
        ...
        for j in range(len(tds)):
            # if j == 0:
            #     address = tds.eq(j)("a").text.txt()
            #     item.append(address)
            #     #print(tds.eq(j)("a").text.txt())
            ele = tds.eq(j).text()
            item.append(ele)
        print(item)
        table.append(item)
...
def getContracts():
    contracts = excelUtil.excel_table_byname("Smartcontract.xls", by_name='Verified Contracts')
